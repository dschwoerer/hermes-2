...
private:
  ...
  BoutReal NLim; // Denisty clamping
  ...
  bool radial_inner_averagey; // Average Ne, Pe, Pi fields in Y in inner radial
                              // buffer
  ...
  bool sheath_allow_supersonic; // If plasma is faster than sound speed, go to
                                // plasma velocity
  bool parallel_sheaths;
  int par_sheath_model;     // Sets parallel boundary condition model
  BoutReal electron_weight; // electron heaviness in units of m_e (for slower
                            // boundaries)
  bool par_sheath_ve;
