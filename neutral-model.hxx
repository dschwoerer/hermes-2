...
public:
   ...
   *
   */
  void setNormalisation(BoutReal Te, BoutReal Ne, BoutReal B, BoutReal length,
                        BoutReal freq) {
    Tnorm = Te;
    Nnorm = Ne;
    Bnorm = B;
   ...
   */
  virtual void update(const Field3D &Ne, const Field3D &Te, const Field3D &Ti,
                      const Field3D &Vi) = 0;

  /*!
   *
   */
  virtual void addDensity(int UNUSED(x), int UNUSED(y), int UNUSED(z),
                          BoutReal UNUSED(dndt)) {}
  virtual void addPressure(int UNUSED(x), int UNUSED(y), int UNUSED(z),
                           BoutReal UNUSED(dpdt)) {}
  virtual void addMomentum(int UNUSED(x), int UNUSED(y), int UNUSED(z),
                           BoutReal UNUSED(dnvdt)) {}

  /*!
   * Preconditioning
   */
  virtual void precon(BoutReal UNUSED(t), BoutReal UNUSED(gamma),
  ...
  Field3D Fperp; // Ion-neutral friction in vorticity. This is a rate,
                 // normalised by Fnorm
...
protected:
  BoutReal Tnorm, Nnorm, Bnorm, Lnorm,
      Fnorm; // Normalisations for temperature, density, magnetic field, lengths
             // and frequencies
  ...
  void neutral_rates(const Field3D &Ne, const Field3D &Te, const Field3D &Ti,
